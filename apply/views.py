...
def apply(request):
    if request.method == 'POST':
        ...
        if first and email and mobile and education and experience and fileresume and job_title:
            print(request.POST)
            print(request.FILES)
            applicant = ApplicantForm.objects.create(job=job_title,name=f'{first} {last}',email=email,mobile=mobile,education=education,
                        exp=experience,resume=fileresume)
